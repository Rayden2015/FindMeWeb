...
function checkTableName($shortTName, $type=false)
{
	...
	if ("users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("user_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("user_jobs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("user_details" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("findme_web_audit" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("find_me_ids" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("verification_ids" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("find_me_jobs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("job_categories" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("regions" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("districts" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("towns" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_stations" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_coordinates_picture" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_coordinates_map" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_coordinates_info" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_coordinates_contact" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("polling_coordinates" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$strPerm = GetUserPermissions("user_types");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="user_types";
	}
	$strPerm = GetUserPermissions("user_jobs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="user_jobs";
	}
	$strPerm = GetUserPermissions("user_details");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="user_details";
	}
	$strPerm = GetUserPermissions("findme web_audit");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="findme web_audit";
	}
	$strPerm = GetUserPermissions("find_me_ids");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="find_me_ids";
	}
	$strPerm = GetUserPermissions("verification_ids");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="verification_ids";
	}
	$strPerm = GetUserPermissions("find_me_jobs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="find_me_jobs";
	}
	...
	$strPerm = GetUserPermissions("towns");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="towns";
	}
	$strPerm = GetUserPermissions("polling_stations");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_stations";
	}
	$strPerm = GetUserPermissions("polling_coordinates_picture");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_coordinates_picture";
	}
	$strPerm = GetUserPermissions("polling_coordinates_map");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_coordinates_map";
	}
	$strPerm = GetUserPermissions("polling_coordinates_info");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_coordinates_info";
	}
	$strPerm = GetUserPermissions("polling_coordinates_contact");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_coordinates_contact";
	}
	$strPerm = GetUserPermissions("polling_coordinates");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="polling_coordinates";
	}
	return $arr;
}
...
function GetUserPermissionsStatic( $table )
{
	...
	if($table=="users" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="users" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="users" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="user_types" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_types" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_types" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="user_jobs" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_jobs" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_jobs" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="user_details" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_details" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="user_details" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="findme web_audit" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="findme web_audit" && $sUserGroup=="2")
	{
			return "".$extraPerm;
	}
	if($table=="findme web_audit" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="findme web_audit")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_ids" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_ids" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_ids" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="find_me_ids")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="verification_ids" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="verification_ids" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="verification_ids" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="verification_ids")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_jobs" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_jobs" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="find_me_jobs" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="find_me_jobs")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="job_categories" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="job_categories" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="job_categories" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="regions" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="regions" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="regions" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="districts" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="districts" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="districts" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="towns" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="towns" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="towns" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="polling_stations" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_stations" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_stations" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="polling_coordinates_picture" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_picture" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_picture" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="polling_coordinates_picture")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_map" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_map" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_map" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="polling_coordinates_map")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_info" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_info" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_info" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="polling_coordinates_info")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_contact" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_contact" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates_contact" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="polling_coordinates_contact")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates" && $sUserGroup=="4")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates" && $sUserGroup=="2")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table=="polling_coordinates" && $sUserGroup=="1")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="polling_coordinates")
	{
		return "AEDSPI".$extraPerm;
	}
