...
require_once("include/polling_coordinates_variables.php");
...
$layout = new TLayout("export2", "BoldOrange", "MobileOrange");
$layout->version = 2;
...
$page_layouts["polling_coordinates_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["polling_coordinates_id"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["user_id"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["polling_coordinates_id"] = urldecode($arr[0]);
			$keys["user_id"] = urldecode($arr[1]);
			$selected_recs[] = $keys;
		}
	}
