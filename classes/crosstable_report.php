...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		if("users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("user_types" == $this->xml_array['tables'][0])
			$isDB = true;
		if("user_jobs" == $this->xml_array['tables'][0])
			$isDB = true;
		if("user_details" == $this->xml_array['tables'][0])
			$isDB = true;
		if("findme web_audit" == $this->xml_array['tables'][0])
			$isDB = true;
		if("find_me_ids" == $this->xml_array['tables'][0])
			$isDB = true;
		if("verification_ids" == $this->xml_array['tables'][0])
			$isDB = true;
		if("find_me_jobs" == $this->xml_array['tables'][0])
			$isDB = true;
		if("job_categories" == $this->xml_array['tables'][0])
			$isDB = true;
		if("regions" == $this->xml_array['tables'][0])
			$isDB = true;
		if("districts" == $this->xml_array['tables'][0])
			$isDB = true;
		if("towns" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_stations" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_coordinates_picture" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_coordinates_map" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_coordinates_info" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_coordinates_contact" == $this->xml_array['tables'][0])
			$isDB = true;
		if("polling_coordinates" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
}
