...
{
	if(count($arr["group_fields"])==0 && $is_save)
		$arr["group_fields"]=$_SESSION["webreports"]["group_fields"];
	if($arr["group_fields"][count($arr["group_fields"])-1]["cross_table"]=="true")
	{
		if(count($arr["group_fields"])-1<2 && $is_save)
		{
		...
		$count_x=0;
		$count_y=0;
		for($i=0;$i<count($arr["group_fields"])-1;$i++)
		{
