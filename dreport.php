...
// Check user permissions
	if(!@$_SESSION["UserID"]) {
		...
		return;
	} elseif ( $rpt_array['settings']['status'] == "private" && $rpt_array['owner'] != @$_SESSION["UserID"] ) {
...
$sWhere="";
if(!$editmode)
	$sWhere = CalcSearchParam(!is_wr_project());
		...
		//								$value.=" src=\"" . $rpt_array['short_table_name'] . "_imager.php?field=" . runner_htmlspecialchars(rawurlencode($fld["strThumbnail"])) . $keylink . "&alt=" . runner_htmlspecialchars(rawurlencode(fldname($fld))) . "".$keylink."\">";
						...
						if($render_mode!=MODE_EXPORT)
						{
							if(CheckImageExtension(db_fld_value($data,fldname($fld))))
							{
								if ( $fld["show_thumbnail"] ) {
									...
									$value.=" border=0";
									$value.=" src=\"".runner_htmlspecialchars(AddLinkPrefix($gSettings, fldname($fld),db_fld_value($data,fldname($fld))))."\">";
								}
							}
						}
					...
					elseif(!is_wr_project())
					{
						$value=WRProcessLargeText($viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, ""),
							fldname($fld),count($rowinfo),100,$render_mode,$fld['label']);
					}
					...
					if(!$grouptotals_count[$arr['name']])
					{
						foreach ( $rpt_array['totals'] as $fld ) 
						{
							if ( $fld['show'] && $fld['min'] ) {
								...
								$grouptotals_dispmin[$arr['name']][fldname($fld)] = 
									$viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
							}
							if ( $fld['show'] && $fld['max'] ) {
					...
					else
					{
						...
						{ // order nReportPageOrder
							if ( $fld['show'] && ( $fld['min'] || $fld['max'] ) ) {
								if ( $fld['min'] ) {
									if ( $grouptotals_min[$arr['name']][fldname($fld)] > db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $grouptotals_min[$arr['name']][fldname($fld)]=="")
									{
										$grouptotals_min[$arr['name']][fldname($fld)] = db_fld_value($data,fldname($fld));
										$grouptotals_dispmin[$arr['name']][fldname($fld)] = 
											$viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
								...
								if ( $fld['max'] ) {
									if ( $grouptotals_max[$arr['name']][fldname($fld)] < db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $grouptotals_max[$arr['name']][fldname($fld)]=="")
									{
										$grouptotals_max[$arr['name']][fldname($fld)] = db_fld_value($data,fldname($fld));
										$grouptotals_dispmax[$arr['name']][fldname($fld)] = 
											$viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
				...
				if ( testAgr( $rpt_array['totals'], "", "min", "max", "" ) > 0 ) {
					if(!$pagetotals_count)
					{
						foreach ( $rpt_array['totals'] as $fld ) {
							if ( $fld['show'] && $fld['min'] ) {
								...
								$pagetotals_dispmin[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
							}
							if ( $fld['show'] && $fld['max'] ) {
								$pagetotals_max[fldname($fld)] = db_fld_value($data,fldname($fld));
								$pagetotals_dispmax[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
							}
						}
					}
					else
					{
						foreach ( $rpt_array['totals'] as $fld ) { // order nReportPageOrder
							if ( $fld['show'] && ( $fld['min'] || $fld['max'] ) ) {
								if ( $fld['min'] ) {
									if($pagetotals_min[fldname($fld)] > db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $pagetotals_min[fldname($fld)]=="")
									{
										$pagetotals_min[fldname($fld)] = db_fld_value($data,fldname($fld));
										$pagetotals_dispmin[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
									}
								...
								if ( $fld['max'] ) {
									if($pagetotals_max[fldname($fld)] < db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $pagetotals_max[fldname($fld)]=="")
									{
										$pagetotals_max[fldname($fld)] = db_fld_value($data,fldname($fld));
										$pagetotals_dispmax[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
									}
			...
			if ( testAgr( $rpt_array['totals'], "", "min", "max", "" ) > 0 ) {
				if(!$globaltotals_count)
				{
					foreach ( $rpt_array['totals'] as $fld ) {
						if ( $fld['show'] && $fld['min'] ) {
							...
							$globaltotals_dispmin[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
						}
						if ( $fld['show'] && $fld['max'] ) {
							$globaltotals_max[fldname($fld)] = db_fld_value($data,fldname($fld));
							$globaltotals_dispmax[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
						}
					}
				}
				else
				{
					foreach ( $rpt_array['totals'] as $fld ) { // order nReportPageOrder
						if ( $fld['show'] && ( $fld['min'] || $fld['max'] ) ) {
							if ( $fld['min'] ) {
								if($globaltotals_min[fldname($fld)] > db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $globaltotals_min[fldname($fld)]=="")
								{
									$globaltotals_min[fldname($fld)] = db_fld_value($data,fldname($fld));
									$globaltotals_dispmin[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
								}
							...
							if ( $fld['max'] ) {
								if($globaltotals_max[fldname($fld)] < db_fld_value($data,fldname($fld)) && db_fld_value($data,fldname($fld))<>"" || $globaltotals_max[fldname($fld)]=="")
								{
									$globaltotals_max[fldname($fld)] = db_fld_value($data,fldname($fld));
									$globaltotals_dispmax[fldname($fld)] = $viewControls->getControl(gfldname($fld), $fld['view_format'])->showDBValue($data, "");
								}
